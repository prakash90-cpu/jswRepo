...
int countWhereCluse=0;
					...
					if(filterArray[countWhereCluse]!=null)
						...
						query = "select distinct username from (select username from "+tableConfig.getProperty("ar_wip")+""
								+ " union all select username from "+tableConfig.getProperty("ar_completed")+") where username!='' ";
...
System.out.println(SelectedValue[0]+"  "+SelectedValue[1]+"  "+SelectedValue[2]);
    ...
    public List getNumberOfCasesCreatedAR(FunctionAppBean functionAppBean) throws SQLException, JSONException {
		...
		assignFunctionAppToArrayAR(functionAppBean);
		...
		String queryWip = " select bg.businessgroup, a.GBL_location ,l.COLOR_NAME,l.LOCATIONDISPLAYNAME,Date(DateCreated) as currentDate, coalesce(count( distinct a.CmAcmCaseidentifier),0) as period "
				+ " from "+tableConfig.getProperty("ar_wip")+" a, businessgroup bg,businessgroup_business bgb,business b,location l "
		...
		String queryCompleted = " select bg.businessgroup, a.GBL_location ,l.COLOR_NAME,l.LOCATIONDISPLAYNAME,Date(DateCreated) as currentDate, coalesce(count( distinct a.CmAcmCaseidentifier),0) as period "
				+ " from "+tableConfig.getProperty("ar_completed")+" a, businessgroup bg,businessgroup_business bgb,business b,location l "
				+ " where   bgb.business_id=b.id and bgb.businessgroup_id=bg.id and l.locationValue=a.Gbl_location "
		...
		// slf4jLogger.info("EXTRACT@@@@@-AR report 1"+query);
		System.out.println("EXTRACT@@@@@-AR report 1" + query);
		Connection conn = null;
		PreparedStatement pstmt = null;
		ArrayList list = null;
		try {
	...
	public String getNumberOfCasesCreatedAR(String columnValueJson) throws SQLException, JSONException {
		System.out.println("aaaaaaaajaxxxxxxxxx" + columnValueJson);

		JSONObject selectedDate = new JSONObject(columnValueJson);

		String queryWip = "select "+tableConfig.getProperty("ar_report1")+" "

				+ " from "+tableConfig.getProperty("ar_wip")+" a";
		String queryCompleted = "select "+tableConfig.getProperty("ar_report1")+""

				+ " from "+tableConfig.getProperty("ar_completed")+" a";
	...
	public void getNumberOfCasesCreatedExcelAR(String columnValueJson) throws SQLException, JSONException {
		System.out.println("aaaaaaaajaxxxxxxxxx" + columnValueJson);
		...
		System.out.println("Restquery " + query);
		generateExcel(query,"AR_Report1","Number of cases created");
	...
	public List getCasesCreatedCompletedAR(FunctionAppBean functionAppBean) throws SQLException, JSONException {
		...
		if (!functionAppBean.getCaseType().isEmpty()) {
			System.out.println(functionAppBean.getCaseType());
		/*if(functionAppBean.getCaseType().equals("AR_BillofExchange")||functionAppBean.getCaseType().equals("AR_ProcessLetterofCredit"))
		{
	*/		commonQuery += " and process in ('" + functionAppBean.getCaseType() + "') ";
		...
		System.out.println("AR report 2" + query);
		Connection conn = null;
		PreparedStatement pstmt = null;
		try {
			conn = dataSource.getConnection();
	...
	public String  getCaseCreatedCompanyCompletedAR(String columnValue) throws SQLException, JSONException {
		...
		String queryWip = "select "+tableConfig.getProperty("ar_report2")+" from "+tableConfig.getProperty("ar_wip")+" where CmAcmCaseidentifier in (select distinct CmAcmCaseidentifier from "+tableConfig.getProperty("ar_wip")+" cc,(select * from process_performance where  Function='AR' "
				+ "  ";
		String queryCompleted = "select "+tableConfig.getProperty("ar_report2")+" from "+tableConfig.getProperty("ar_completed")+" where CmAcmCaseidentifier in (select distinct CmAcmCaseidentifier from "+tableConfig.getProperty("ar_completed")+" cc,(select * from process_performance where  Function='AR' "
				+ "  ";
		String query ="";
		// Case Type Where clause
		if (selectedDate.has("process")) {
			
			query += " and process in ('" + selectedDate.getString("process") + "') ";
		...
		// Case Type Where clause
		if (selectedDate.has("process")) {
			query += " and CmAcmCaseidentifier LIKE '" +  selectedDate.getString("process") + "%' ";
	...
	public void  getCaseCreatedCompanyCompletedExcelAR(String columnValue) throws SQLException, JSONException {
		...
		generateExcel(query,"AR_Report2","Cases completed upto a step");
...
try {
	/*		ArrayList<String> casetypeList=new ArrayList<String>();
			...
			String sql=" select caseTypeDisplayName from CaseType where functions='AR'";
			...
			String query = "select Process,stagename,queuename, stepname,response from process_performance where function='AR' ";
	
			 // Process Where clause
			 
				  
					/*if(casetypeList.get(countProcess).equals("AR_BillofExchange")||casetypeList.get(countProcess).equals("AR_ProcessLetterofCredit"))
					{*/
			if (!functionAppBean.getCaseType().isEmpty()) {
			...
			for (int countOuter = 0; countOuter < jsonProcessPerformance.length(); countOuter++) {
				...
				String queryWip = "select UserName,count(distinct CmAcmCaseidentifier ) as count,Date(endTime) as currentDate from "+tableConfig.getProperty("ar_wip")+" where username is not null and username!=''  and is_active='Y' ";
	
				String queryCompleted = "select UserName,count(distinct CmAcmCaseidentifier ) as count,Date(endTime) as currentDate from "+tableConfig.getProperty("ar_completed")+" where username is not null and username!='' ";
			...
			*/	
				...
				// slf4jLogger.info("EXTRACT@@@@@-AR report 3"+query);
				System.out.println("EXTRACT@@@@@-AR report 3" + query);
	...
	public String  getCasesProcessedAR(String columnValue) throws SQLException, JSONException {
		...
		String queryWip = "select "+tableConfig.getProperty("ar_report3")+" from "+tableConfig.getProperty("ar_wip")+" where CmAcmCaseidentifier in (  select distinct CmAcmCaseidentifier from "+tableConfig.getProperty("ar_wip")+" cc,";
		String queryCompleted = "select "+tableConfig.getProperty("ar_report3")+" from "+tableConfig.getProperty("ar_completed")+"  where CmAcmCaseidentifier in (select distinct CmAcmCaseidentifier from "+tableConfig.getProperty("ar_completed")+" cc,";

		String query = "";
		/*if( selectedDate.getString("process") .equals("AR_BillofExchange")|| selectedDate.getString("process") .equals("AR_ProcessLetterofCredit"))					{
		*/	query = "process_performance pp where cc.QueueName=pp.QueueName and cc.StepName=pp.StepName and cc.Response =pp.Response";
		...
		/*}
			...
			query = "process_performance pp where  GBL_caseStatus =pp.Response";
	...
	public void  getCasesProcessedExcelAR(String columnValue) throws SQLException, JSONException {
		...
		*/	query = "process_performance pp where cc.QueueName=pp.QueueName and cc.StepName=pp.StepName and cc.Response =pp.Response";
		...
		generateExcel(query,"AR_Report3","Cases processed by user");
	}
...
public List getRoleWiseTimeTakenAR(FunctionAppBean functionAppBean) throws SQLException, JSONException {
		...
		/*String stringQueryCount[] = { "CAST(CAST(sum(TIMESTAMPDIFF(2,CHAR(endTime-startTime))) AS DECIMAL(18,4) )/3600 AS DECIMAL(18,4) ) ", "CAST(CAST(sum(TIMESTAMPDIFF(2,CHAR(endTime-startTime))) AS DECIMAL(18,4) )/3600 AS DECIMAL(18,4) )",
	...
	*/
					queryCompleted += ",Date(endTime) as currentDate from "
							...
							+ " c"
							+ " ,roles r "
							+ " where  r.arroles=c.queuename and r.functions='AR' ";
				...
				// slf4jLogger.info("EXTRACT@@@@@-AR report 1"+query);
				System.out.println("EXTRACT@@@@@-AR report 4" + query);
				conn = dataSource.getConnection();
				 pstmt = conn.prepareStatement(query);
				ResultSet resultSet = pstmt.executeQuery();
	           ...
	           query +=" ,CAST(CAST(sum(TIMESTAMPDIFF(2,CHAR(endTime-startTime))) AS DECIMAL(18,8) )/3600 AS DECIMAL(18,4) ) as count ";
	            ...
	            query +=", CAST((CAST(sum(TIMESTAMPDIFF(2,CHAR(endTime-startTime))) AS DECIMAL(18,8) )/3600 )/count( distinct CmAcmCaseidentifier)AS DECIMAL(18,4) ) as count ";
	             ...
	             query +=", count( distinct CmAcmCaseidentifier) as count ";
						query +="from "
						+ tableConfig.getProperty("ar_completed")
						+ " c ,roles r  where   r.arroles=c.queuename and r.functions='AR' "+queryTotal+"   group by queuename,r.alias_name  order by queuename";
					//	System.out.println("EXTRACT@@@@@-AR report 4" + query);
						pstmt = conn.prepareStatement(query);
				resultSet = pstmt.executeQuery();
				int countList=0;
	...
	public String  getRoleWiseTimeTakenAR(String columnValue) throws SQLException, JSONException {
		...
		String queryCompleted = "select "+tableConfig.getProperty("ar_report4")+" from "+tableConfig.getProperty("ar_completed")+" where CmAcmCaseidentifier in (select distinct CmAcmCaseidentifier from "+tableConfig.getProperty("ar_completed")+" ";
	...
	public void  getRoleWiseTimeTakenExcelAR(String columnValue) throws SQLException, JSONException {
		...
		generateExcel(query,"AR_Report4","Role wise time taken to complete process");
	...
	public List getTotalCycleTimeAR(FunctionAppBean functionAppBean) throws SQLException, JSONException {
		...
		String stringQueryCount[] = { " CAST(CAST(sum(TIMESTAMPDIFF(2,CHAR(endTime-startTime))) AS DECIMAL(18,6) )/3600 AS DECIMAL(18,4) ) ", " CAST(CAST(sum(TIMESTAMPDIFF(2,CHAR(endTime-startTime))) AS DECIMAL(18,6) )/3600 AS DECIMAL(18,4) ) ",
		"count( distinct CmAcmCaseidentifier )" };
        List mainList = new ArrayList();

for (int countQuery = 0; countQuery < stringQueryCount.length; countQuery++) {
		Connection conn = null;
		PreparedStatement pstmt = null;
		try {
			...
			queryWip += " from "
					+ tableConfig.getProperty("ar_wip")
					+ " cc,process_performance pp where pp.Function='AR' and cc.QueueName=pp.QueueName and cc.StepName=pp.StepName and cc.Response =pp.Response and CmAcmCaseidentifier LIKE  CONCAT(process,'%') and endTime is not null and is_active='Y'  ";
			...
			queryCompleted += " from "
					+ tableConfig.getProperty("ar_completed")
					+ " cc ,process_performance pp where pp.Function='AR' and cc.QueueName=pp.QueueName and cc.StepName=pp.StepName and cc.Response =pp.Response and CmAcmCaseidentifier LIKE  CONCAT(process,'%')";
			...
			// slf4jLogger.info("EXTRACT@@@@@-AR report 5"+query);
			System.out.println("EXTRACT@@@@@-AR report 5" + query);
           ...
           query +=" ,CAST(CAST(sum(TIMESTAMPDIFF(4,CHAR(endTime-startTime))) AS DECIMAL(18,2) )/60 AS DECIMAL(18,4) ) as count ";
            ...
            query +=", CAST((CAST(sum(TIMESTAMPDIFF(4,CHAR(endTime-startTime))) AS DECIMAL(18,6) )/60 )/count( distinct CmAcmCaseidentifier)AS DECIMAL(18,4) ) as count ";
             ...
             query +=", count( distinct CmAcmCaseidentifier) as count ";
			query+="from ("
					+ " select stagename,alias_name,CmAcmCaseidentifier,starttime,endtime from "+tableConfig.getProperty("ar_wip")+"  " +
							"cc,process_performance pp where pp.Function='AR' and cc.QueueName=pp.QueueName and cc.StepName=pp.StepName and cc.Response =pp.Response and CmAcmCaseidentifier LIKE  CONCAT(process,'%') and endTime is not null  and is_active='Y'"
                      + totalQuery+" union all  select stagename,alias_name,CmAcmCaseidentifier,starttime,endtime from "+tableConfig.getProperty("ar_completed")+" cc,process_performance pp where pp.Function='AR' and cc.QueueName=pp.QueueName and cc.StepName=pp.StepName and cc.Response =pp.Response and CmAcmCaseidentifier LIKE  CONCAT(process,'%') and endTime is not null  "
                              + totalQuery
	...
	public String  getTotalCycleTimeAR(String columnValue) throws SQLException, JSONException {
		...
		String queryWip = "select "+tableConfig.getProperty("ar_report5")+" from "+tableConfig.getProperty("ar_wip")+" where CmAcmCaseidentifier in (select distinct CmAcmCaseidentifier from "+tableConfig.getProperty("ar_wip")+" ";
		String queryCompleted = "select "+tableConfig.getProperty("ar_report5")+" from "+tableConfig.getProperty("ar_completed")+" where CmAcmCaseidentifier in  (select distinct CmAcmCaseidentifier from "+tableConfig.getProperty("ar_completed")+" ";
		String query = " cc,process_performance pp where pp.Function='AR' and cc.QueueName=pp.QueueName and cc.StepName=pp.StepName and cc.Response =pp.Response and CmAcmCaseidentifier LIKE  CONCAT(process,'%') and endTime is not null "
				;
// StageName Where clause
if (selectedDate.has("StageName")) {
...
if (selectedDate.has("StageName")) {
	query += " and StageName in ('" + selectedDate.getString("StageName") + "') ";
		...
		generateExcel(query,"AR_Report5","Total Cycle time taken ");
...
*/
			...
			query += ", case when a1.pendingDays between " + 0 + " and " + 1
					+ " then 1 else 0 end as b0";
			query += ", case when a1.pendingDays between " + 1 + " and " + 3
			...
			query += ", case when a1.pendingDays between " + 30 + " and " + 90
					...
					query += ", case when a1.pendingDays>" + 7 + " then 1 else 0 end as b3";
		...
		query += " select a.queuename,r.alias_name,  coalesce(CEILING(CAST(TIMESTAMPDIFF(2,CHAR(current Timestamp -dateCreated)) AS DECIMAL(18,2))/86400),0) as pendingDays "
				+ " from "+tableConfig.getProperty("ar_wip")+" a,roles r where   "
				+ "  endTime is null and r.arroles=queuename and functions='AR' and is_active='Y'  ";
		...
		/*query += 
				...
				+ functionAppBean.getToDate() + " 23:59:59' ";
	*/	
		query += "  group by queuename,r.alias_name ,CmAcmCaseidentifier,CEILING(CAST(TIMESTAMPDIFF(2,CHAR(current Timestamp -dateCreated)) AS DECIMAL(18,2))/86400)";
		...
		// slf4jLogger.info("EXTRACT@@@@@-AR report 1"+query);
		System.out.println("EXTRACT@@@@@-AR report 6" + query);
		Connection conn = null;
		PreparedStatement pstmt = null;
		ArrayList list = null;
		try {
...
*/
	...
	/*	String dateCreatedBucket="";
		...
		*/
		...
		String query = "select "+tableConfig.getProperty("ar_report6")+",CEILING(CAST(TIMESTAMPDIFF(2,CHAR(current Timestamp -dateCreated)) AS DECIMAL(18,2))/86400) as Bucket from "+tableConfig.getProperty("ar_wip")+" where CmAcmCaseidentifier in  (select distinct CmAcmCaseidentifier from "+tableConfig.getProperty("ar_wip")+" where endTime is null  and is_active='Y' ";
		...
		if (selectedDate.has("CaseType")) {
		  ...
		  query += " and  CEILING(CAST(TIMESTAMPDIFF(2,CHAR(current Timestamp -dateCreated)) AS DECIMAL(18,2))/86400) between "+selectedDate.getString("bucketFrom")+" and "+selectedDate.getString("bucketTo");
			  ...
			  query += " and  CEILING(CAST(TIMESTAMPDIFF(2,CHAR(current Timestamp -dateCreated)) AS DECIMAL(18,2))/86400) >"+selectedDate.getString("bucketFrom");
		  }  
	...
	public void  getCasesPendingExcelAR(String columnValue) throws SQLException, JSONException {
		...
		*/
		...
		generateExcel(query,"AR_Report6","Number of cases pending from creation date");
		}
	 ...
	 public List getCasesCompletedCumulativelyAR(FunctionAppBean functionAppBean)  throws SQLException, JSONException{
		 assignFunctionAppToArrayAR(functionAppBean);
			...
			String whereClauseCondition[]={""," and CmAcmCaseidentifier LIKE 'AR_ProcessLetterofCredit%' and response='Amendment Required' "," and CmAcmCaseidentifier LIKE 'AR_ProcessLetterofCredit%' and  response='Amendment  not Required' ","  and CmAcmCaseidentifier LIKE 'AR_BillofExchange%' and queuename='AR_ProcessAssociateBOE' and stepname='Verify BOE & Cover Letter' and response='Prepare Supportings' "};
			String whereClauseName[]={"For Others (Number of cases completed cumulatively)"," For LC  (cases in which Amendment has been Requested) "," For LC (cases in which Amendment has not been Requested) "," For BOE (Cases Completed till stage 'BOE Creation' )"};
			
			for(int clauseCount=0;clauseCount<whereClauseCondition.length;clauseCount++){
			
				FunctionApplication innerOuter = new FunctionApplication();
				innerOuter.setName(whereClauseName[clauseCount]);
				innerOuter.setSelectedValue(""+clauseCount);
			String query = "select a2.BusinessGroup, a2.GBL_location ,a2.COLOR_NAME,a2.LocationDisplayName";
			String groupByDate = "";
				...
				query += ", case when a1.pendingDays between " +0+ " and " +1+ " then 1 else 0 end as b0"
						;
				query += ", case when a1.pendingDays between " +0+ " and " +3+ " then 1 else 0 end as b1"
						;
				query += ", case when a1.pendingDays between " +0+ " and " +7+ " then 1 else 0 end as b2"
						;
			...
			query += " select bg.BusinessGroup, GBL_location,l.COLOR_NAME,l.LocationDisplayName, coalesce( CEILING(CAST(TIMESTAMPDIFF(2,CHAR(DateLastmodified-DateCreated)) AS DECIMAL(18,2))/86400) ,0) as pendingDays "
					+ " from "+tableConfig.getProperty("ar_completed")+" a,business b,businessGroup bg,location l, businessGroup_business bgb where b.id=bgb.business_id and l.locationValue= GBL_location and bg.id=bgb.businessGroup_id and b.companyValue=gbl_CompanyName "
					+ "  "+whereClauseCondition[clauseCount]+" ";
			...
			System.out.println("EXTRACT@@@@@-AR report 7" + query);
			Connection conn = null;
			PreparedStatement pstmt = null;
			try {
				conn = dataSource.getConnection();
				...
				while (resultSet.next()) {
					...
					innerJson.setCount("0");
						...
						innerOuter.value.add(innerJson);
...
public String  getCasesCompletedCumulativelyAR(String columnValueJson) throws SQLException, JSONException {
JSONObject selectedDate = new JSONObject(columnValueJson);

	
	
String queryCompleted = "select "+tableConfig.getProperty("ar_report7")+", CEILING(CAST(TIMESTAMPDIFF(2,CHAR(DateLastmodified-DateCreated)) AS DECIMAL(18,2))/86400) as Bucket from "+tableConfig.getProperty("ar_completed")+" where CmAcmCaseidentifier in (select distinct CmAcmCaseidentifier from "+tableConfig.getProperty("ar_completed")+" a";
		String query= ", business b , businessgroup_business bgb  ,"
				+ "businessgroup bg where  bgb.business_id=b.id and b.companyValue=a.Gbl_companyName and bg.id=bgb.businessgroup_id  ";

		

if(selectedDate.getString("cases").equals("1"))
	query +=" and CmAcmCaseidentifier LIKE 'AR_ProcessLetterofCredit%' and response='Amendment Required' ";
else if(selectedDate.getString("cases").equals("2"))
		query +=" and CmAcmCaseidentifier LIKE 'AR_ProcessLetterofCredit%' and  response='Amendment  not Required' ";
else if(selectedDate.getString("cases").equals("3"))
			query +="  and CmAcmCaseidentifier LIKE 'AR_BillofExchange%' and queuename='AR_ProcessAssociateBOE' and stepname='Verify BOE & Cover Letter' and response='Prepare Supportings' ";
		...
		if (selectedDate.has("CaseStatus")) {
		  ...
		  query += " and   CEILING(CAST(TIMESTAMPDIFF(2,CHAR(DateLastmodified-DateCreated)) AS DECIMAL(18,2))/86400)  between "+selectedDate.getString("bucketFrom")+" and "+selectedDate.getString("bucketTo");
			  ...
			  query += " and  CEILING(CAST(TIMESTAMPDIFF(2,CHAR(DateLastmodified-DateCreated)) AS DECIMAL(18,2))/86400)  >7 ";
...
public void  getCasesCompletedCumulativelyExcelAR(String columnValueJson) throws SQLException, JSONException {
JSONObject selectedDate = new JSONObject(columnValueJson);
...
query += " and endTime between '" + selectedDate.getString("dateFrom") + " 00:00:00' and '"
		...
		generateExcel(query,"AR_Report7","Cases completed cumulatively");
...
*/
	...
	// slf4jLogger.info("EXTRACT@@@@@-AR report 1"+query);
	System.out.println("EXTRACT@@@@@-AR report 8" + query);
	Connection conn = null;
	PreparedStatement pstmt = null;
	ArrayList list = null;
	try {
...
public String  getCasesPendingForProcesssingAR(String columnValueJson) throws SQLException, JSONException {
	JSONObject selectedDate = new JSONObject(columnValueJson);


	String query = "select "+tableConfig.getProperty("ar_report8")+",CEILING(CAST(TIMESTAMPDIFF(2,CHAR(current Timestamp -startTime)) AS DECIMAL(18,2))/86400) as Bucket from "+tableConfig.getProperty("ar_wip")+" where CmAcmCaseidentifier in (select distinct CmAcmCaseidentifier from "+tableConfig.getProperty("ar_wip")+" where endTime is null and is_active='Y'  ";
...
public void  getCasesPendingForProcesssingExcelAR(String columnValueJson) throws SQLException, JSONException {
	JSONObject selectedDate = new JSONObject(columnValueJson);
	...
	generateExcel(query,"AR_Report8","Number of cases pending from last role start time");
	
}	
	    
private void assignFunctionAppToArrayAR(FunctionAppBean functionAppBean){
