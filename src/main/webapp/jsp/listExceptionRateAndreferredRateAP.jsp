...
<h2 class="w3-center w3-xxlarge"><i>AP</i></h2>
					...
					<b>Report 9 - Exception Report and First Pass Rate</b>
					</h4>	
				

<form:form commandName="FunctionAppBean" action="/jsw-report/ApController/listExceptionRateAndReferredRateAP">				
...
<c:choose>
     <c:when test="${functionName.name=='Location'}">
        <tr><td style="width:15%"><label>Date From </label></td><td style="width:30%"><form:input  style="width:70%" type="date" id="dateFrom"  onChange="setToBucketFromDate(this)" path="fromDate" /> (mm/dd/yyyy)</td>
       <td style="width:15%"><label>Date To </label></td><td style="width:30%"><form:input  style="width:70%" type="date"  path="toDate" id="dateTo" onchange="setToBucketToDate()" /> (mm/dd/yyyy)</td></tr>
    </c:when>    
...
<div class='w3-container w3-round-xxlarge' style="background-color:#e3ecf9">
 ...
 <table  class="w3-table-all w3-hoverable w3-striped w3-border  w3-center processTable process" stepname="${listOuterCases.name}">
					<thead>
						...
						<span title="download table"  onclick="getCaseCreatedAll('listCaseCreated<%=countGraphId++ %>', 'AP Exception Rate and First Pass Rate')" class="w3-badge fa fa-download w3-blue  w3-right w3-hover-green"></span>
...
<tr class="busi">
   ...
   <td class="w3-center firstTd">${listCases.name}</td>
								...
								<u onclick="getCaseStudiedTableByUserName(this,'getExceptionAndFirstPassAP',0);" 
								 class="w3-hover-blue w3-hover-text-black" 
								>${listCases.count}</u>
									
								<span  onclick="getCaseStudiedTableByUserName(this,'getExceptionAndFirstPassExcelAP',1);" class="fa fa-download  w3-right w3-text-blue w3-hover-blue">
...
function graphNumberOfCaseProcess(chartName,radioData){
	 ...
	 var title;
		...
		x : {title : 'Cases ', titleDistance: 25},
          ...
          if(count==0){
       		 $("#titleDiv").remove(); 
                $(chartName).prepend('<html><div id="titleDiv"> <h4 style="color:purple">Title :  Exception Report and First Pass Rate</h5></div></html>');
       			  }
