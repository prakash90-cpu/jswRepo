			...
			<div id="myTableList">
				<div class="w3-container">
					...
					<h3 class="w3-text-blue">
						<b>Number of Cases Processed</b>
					</h3>
				</div>
 
<form:form commandName="FunctionAppBean" action="/jsw-report/mainController/submitNumberOfCasesProcessedCORP">				
				<table
...
<c:choose>
    ...
    <tr><td><label>Date From </label></td><td><form:input type="date" path="fromDate" /> (mm/dd/yyyy)</td>
       <td><label>Date To </label></td><td><form:input type="date" path="toDate" /> (mm/dd/yyyy)</td></tr>
   </c:when>    
...
<p>
		...
		<c:if test="${not empty listCasesCreated}"> 
			...
			<c:forEach var="listOuterCases" items="${listCasesCreated}">							
					...
					Number cases ${listOuterCases.name} by the users
...
<tr>
  ...
  <c:when test="${not empty listCases.name}">
   <td class="w3-center">${listCases.name}</td>
  </c:when>
  <c:otherwise>
