...
<h2 class="w3-center w3-xxlarge"><i>RTR</i></h2>
					<h4 class="w3-text-blue">
							<b>Report 5- Total cycle time taken to complete the process
</b>
					</h4>
<form:form commandName="FunctionAppBean" action="/jsw-report/RTRController/listTotalCycleTimeRTR">				
					<table
...
<c:choose>
 ...
 <input onChange="$('#listCaseCreated<%=countTimeTaken %>').hide();$('#chartContainer<%=countTimeTaken %>').show();$('#table-btn').show();graphOfTotalTimeTaken($('#chartContainer<%=countTimeTaken %>'),<%=countTimeTaken %>);" class="w3-radio" type="radio" name="graph<%=countTimeTaken %>">
		 ...
		 <div id="chartContainer<%=countTimeTaken %>" class="w3-center" style="height: 500px; width:80%;display:none"></div>
    		...
    		<h5 class="w3-left"><b> Total <%=timeTaken[countTimeTaken] %> </b></h5>
    		  <p class="w3-right currentDate" id=""></p>
<table 
					class="w3-table-all w3-hoverable w3-striped w3-border  w3-center totalTimeTable" id='<%=timeTaken[countTimeTaken] %>'>
					<thead>
   ...
   <c:forEach items="${listPeriod}" var="value">
						...
						<span  onclick="getCaseCreatedAll('<%=timeTaken[countTimeTaken++] %>', 'RTR  Total Cycle time taken to complete process')" class="w3-badge fa fa-download  w3-right w3-hover-blue"></span>
...
<td class="w3-center firstTd bus${listCases.name}" >${listCases.name}</td>
											 ...
											 <c:choose>
        <c:when test="${fn:contains(FunctionAppBean.getDisplayBy(), '7')}">
		<td class="w3-center" style='cursor:pointer' onClick="getCaseStudiedTableByStageRow(this,'getTotalCycleTimeRTR',0);">${listCases.day1}</td>
										<td style='cursor:pointer' class="w3-center" onClick="getCaseStudiedTableByStageRow(this,'getTotalCycleTimeRTR',0);">${listCases.day2}</td>
											<td style='cursor:pointer' class="w3-center"  onClick="getCaseStudiedTableByStageRow(this,'getTotalCycleTimeRTR',0);">${listCases.day3}</td>
													<td style='cursor:pointer' class="w3-center"  onClick="getCaseStudiedTableByStageRow(this,'getTotalCycleTimeRTR',0);">${listCases.day4}</td>
										<td style='cursor:pointer' class="w3-center"  onClick="getCaseStudiedTableByStageRow(this,'getTotalCycleTimeRTR',0);">${listCases.day5}</td>
											<td class="w3-center" style='cursor:pointer' onClick="getCaseStudiedTableByStageRow(this,'getTotalCycleTimeRTR',0);">${listCases.day6}</td>
						
							<td style='cursor:pointer' class="w3-center"  onClick="getCaseStudiedTableByStageRow(this,'getTotalCycleTimeRTR',0);">${listCases.day7}</td>
						
    </c:when>  
        <c:when test="${fn:contains(FunctionAppBean.getDisplayBy(), '4')}">
   		<td style='cursor:pointer' class="w3-center"  onClick="getCaseStudiedTableByStageRow(this,'getTotalCycleTimeRTR',0);">${listCases.week1}</td>
										<td style='cursor:pointer' class="w3-center"  onClick="getCaseStudiedTableByStageRow(this,'getTotalCycleTimeRTR',0);">${listCases.week2}</td>
											<td style='cursor:pointer' class="w3-center"  onClick="getCaseStudiedTableByStageRow(this,'getTotalCycleTimeRTR',0);">${listCases.week3}</td>
													<td style='cursor:pointer' class="w3-center"  onClick="getCaseStudiedTableByStageRow(this,'getTotalCycleTimeRTR',0);">${listCases.week4}</td>
											
											
    </c:when>  
        <c:when test="${fn:contains(FunctionAppBean.getDisplayBy(), '3')}">
   		<td style='cursor:pointer' class="w3-center"  onClick="getCaseStudiedTableByStageRow(this,'getTotalCycleTimeRTR',0);">${listCases.period1}</td>
										<td style='cursor:pointer' class="w3-center"  onClick="getCaseStudiedTableByStageRow(this,'getTotalCycleTimeRTR',0);">${listCases.period2}</td>
											<td style='cursor:pointer' class="w3-center"  onClick="getCaseStudiedTableByStageRow(this,'getTotalCycleTimeRTR',0);">${listCases.period3}</td>
    </c:when>  
								...
								<u class="w3-hover-blue w3-hover-text-black"  onClick="getCaseStudiedTableByStage(this,'getTotalCycleTimeRTR',0);"
								...
								<span onClick="getCaseStudiedTableByStage(this,'getTotalCycleTimeExcelRTR',1);"  class="fa fa-download generateXLS w3-right"></span>
...
for(var count=1;count<totalTd;count++){
	...
	contEachTd+= parseFloat( $(this).find('td:eq('+count+')').text());
	  ...
	  $(this).find(".roleRow").parent().find("tr").last().find('td:eq('+count+')').append("<br><a  style='cursor:pointer'>"+contEachTd.toFixed(2)+"</a>");
	 ...
	 $(".totalTimeTable").eq(countTable).each(function(){	
	  ...
	  var stageValue=stageData(radioData);
	    ...
	    chartName.chart({
	    					...
	    					tooltips : toolTipData(radioData),
	    					values : {
	    						serie1 : thevalues
	    					},
	    					defaultSeries : {
