...
<h2 class="w3-center w3-xxlarge"><i>AP</i></h2>
					<h4 class="w3-text-blue">
						<b>Report 6-Number of Cases pending for processing (Creation/Received date)
</b>
					</h4>
				


<form:form commandName="FunctionAppBean" action="/jsw-report/ApController/listNumberOfCasesPendingAP">				
...
<c:choose>
   <%--  <c:when test="${functionName.name=='Location'}">
     ...
     <c:when test="${functionName.name=='CaseType'}">
      <tr> <td><label>Aging By </label></td>
        
        <td>
        <select  name="AgingBy"  style="width:50%">
				
				<c:forEach items="${agingList}" var="value">
...
<br /> <br />
    ...
    <div class="w3-left"><label>Tabular</label>	
		...
		<input onChange="$('#listCaseCreated').hide();$('#chartContainer').show();graphNumberOfCasePending();" class="w3-radio" type="radio" name="graph">
...
<table 
					class="w3-table-all w3-hoverable w3-striped w3-border w3-small w3-center">
						...
						<tr class="w3-text-white w3-blue w3-center w3-medium" id="firstTr">
				...
				<%-- 		
						 <c:choose>
        <c:when test="${fn:contains(FunctionAppBean.getDisplayBy(), '3/7/15/30/90/90+')}">
  ...
  <%--   </c:when>  
   ...
   --%>
						...
						<span  onclick="getCaseCreatedAll('listCaseCreated', 'AP Number of Cases Pending')" class="w3-badge fa fa-download  w3-right w3-hover-blue"></span>
...
<% String groupName=""; %>
						...
						<c:forEach var="listCases" items="${listCasesCreated}">
							<tr class="roleName" aliasName="${listCases.alias}">
...
<td class="w3-center firstTd bus${listCases.name}" colorName="${listCases.color}">${listCases.name}</td>
		...
		<td class="w3-center" style='cursor:pointer' onClick="getCaseStudiedTableByRoleRow(this,'getCasesPendingAP',0)">${listCases.bucket1}</td>
										<td style='cursor:pointer' class="w3-center" onClick="getCaseStudiedTableByRoleRow(this,'getCasesPendingAP',0)">${listCases.bucket2}</td>
											<td style='cursor:pointer' class="w3-center"  onClick="getCaseStudiedTableByRoleRow(this,'getCasesPendingAP',0)">${listCases.bucket3}</td>
													<td style='cursor:pointer' class="w3-center"  onClick="getCaseStudiedTableByRoleRow(this,'getCasesPendingAP',0)">${listCases.bucket4}</td>
										<td style='cursor:pointer' class="w3-center"  onClick="getCaseStudiedTableByRoleRow(this,'getCasesPendingAP',0)">${listCases.bucket5}</td>
									<td style='cursor:pointer' class="w3-center"  onClick="getCaseStudiedTableByRoleRow(this,'getCasesPendingAP',0)">${listCases.bucket6}</td>	
						
<%--     </c:when>  
        <c:when test="${fn:contains(FunctionAppBean.getDisplayBy(), '15/30/45/45+')}">
   		<td style='cursor:pointer' class="w3-center"  onClick="getCaseStudiedTableByRoleRow(this,'getCasesPendingAP',0)">${listCases.bucket1}</td>
										<td style='cursor:pointer' class="w3-center"  onClick="getCaseStudiedTableByRoleRow(this,'getCasesPendingAP',0)">${listCases.bucket2}</td>
											<td style='cursor:pointer' class="w3-center"  onClick="getCaseStudiedTableByRoleRow(this,'getCasesPendingAP',0)">${listCases.bucket3}</td>
													<td style='cursor:pointer' class="w3-center"  onClick="getCaseStudiedTableByRoleRow(this,'getCasesPendingAP',0)">${listCases.bucket4}</td>
 	...
 	<td style='cursor:pointer' class="w3-center"  onClick="getCaseStudiedTableByRoleRow(this,'getCasesPendingAP',0)">${listCases.bucket4}</td>
				 ...
				 --%>							
								...
								<u class="w3-hover-blue w3-hover-text-black"  onClick="getCaseStudiedTableByRole(this,'getCasesPendingAP',0)"
								...
								<span onClick="getCaseStudiedTableByRole(this,'getCasesPendingExcelAP',1)"  class="fa fa-download  w3-right"></span>
  			 ...
  			 <c:choose>
        <c:when test="${fn:contains(FunctionAppBean.getDisplayBy(), '3/7/11/15/15+')}">
          <td class="w3-center" onClick="getCaseStudiedTableByTotalRow(this,'getCasesPendingAP',0)" >Total of all Role </td>
  			<td class="w3-center" onClick="getCaseStudiedTableByTotalRow(this,'getCasesPendingAP',0)">Total of all Role </td>
  			<td class="w3-center" onClick="getCaseStudiedTableByTotalRow(this,'getCasesPendingAP',0)">Total of all Role </td>
  				<td class="w3-center" onClick="getCaseStudiedTableByTotalRow(this,'getCasesPendingAP',0)">Total of all Role </td>   
  			<td class="w3-center" onClick="getCaseStudiedTableByTotalRow(this,'getCasesPendingAP',0)">Total of all Role </td>
      ...
      <td class="w3-center" onClick="getCaseStudiedTableByTotalRow(this,'getCasesPendingAP',0)">Total of all Role </td>
  			...
  			<td class="w3-center" onClick="getCaseStudiedTableByTotalRow(this,'getCasesPendingAP',0)">Total of all Role </td>
    			...
    			<td class="w3-center" onClick="getCaseStudiedTableByTotal(this,'getCasesPendingAP',0)" >Total of all column on totals  <span   class="fa fa-download generateXLS w3-right"></span></td>
...
var currentBucket=$(thisVariable).parent().parent().find("tr:first").children('td:eq('+$(thisVariable).index()+')').text();
 ...
 //  alert(currentBucket.split(" ")[2]);
  //  alert(currentBucket.split(" ")[4]);
...
var count=0;
 ...
 function graphNumberOfCasePending(){
