...
<body ng-App="myApp" ng-Controller="myCtrl">
	...
	<%@ include file="Header.jsp"%>
		...
		<form:form class="w3-container" commandName="BusinessGroupLocBeanByID" method="POST"  action="/jsw-report/master/submitBgGroupLoc" >
...
<b>
           ...
           <c:forEach items="${BusinessGroupBean}" var="value">
               <c:choose>
            <c:when test="${BusinessGroupLocBeanByID.getBusinessGroup()==value.businessGroup}">
        <option value="${value.id}" selected>${value.businessGroup}</option>
           ...
           <c:otherwise>
            ...
            <p>
			 ...
			 <c:choose>
            <c:when test="${BusinessGroupLocBeanByID.getLocation()==value.locationName}">
     <form:checkbox path="locId" value="${value.locationId}" class="w3-check" checked='checked'/>${value.locationName} <br /> 
          </c:when>
           <c:otherwise>
