...
<form:form commandName="FunctionAppBean" action="/jsw-report/CommController/listNumberOfCasesProcessedCOMM">				
...
<br /> <br />
     ...
     <!-- <div class="w3-left"><label>Tabular</label>	
		<input type="radio" onChange="$('#listCaseCreated').show();$(' #chartContainer').hide();$('#graph-btn').hide();" class="w3-radio" style="padding-right:20px" checked name="graph" readonly>
		<label>Graphical</label>	

		<input onChange="$('#listCaseCreated').hide();$('#chartContainer').show();graphNumberOfCaseProcess();$('#graph-btn').show();" class="w3-radio" type="radio" name="graph" readonly>
		</div> --><br><br />
	<center>
	
		 <div id="chartContainer"  display:none">
		 <!-- style="height: 550px; width:80%; -->
	
			</div>	</center>

  
			<!-- <button class="w3-btn" id="graph-btn" onClick="saveGraph('chartContainer')" style="display:none" >Save graph </button>	 -->
		...
		<%-- 	<c:if test="${settingValue==2}"> 
			...
			<%-- 	<c:forEach var="listCaseByCase" items="${listCasesCreated}">	
		 --%>
    <p class="w3-right currentDate" id=""></p>
   
    <div id='listCasesCreated'>

  
  
<%-- <h5><b>Process :  ${listCaseByCase.name}</b></h5> --%>
<table 
					class="w3-table-all w3-hoverable w3-striped w3-border  w3-center" process="${listCaseByCase.name} ">
					
						<tr class="w3-center " id="firstTr" style="background-color:#372d87;color:white">
						
						
							<td class="w3-center">User Name / Stage Name</td>
   ...
   <c:forEach items="${listPeriod}" var="value">
						...
						<td class="w3-center">Count
						
						
						<span   onclick="getCaseCreatedAll('listCasesCreated', 'Commercial Number of Case Processed by user')"  class="w3-badge fa fa-download  w3-right w3-hover-blue"></span>
 ...
 if(groupName.equals((String)pageContext.getAttribute("groupName")) ){}
  ...
  %>
  			...
  			<td class="busi" >${listCases.name}</td>
  				 ...
  				 <c:choose>
        <c:when test="${fn:contains(FunctionAppBean.getDisplayBy(), '7')}">
       	<td class="w3-center">Total of all ${listCases.name} User</td>
  			<td class="w3-center">Total of all ${listCases.name} User</td>
  			<td class="w3-center">Total of all ${listCases.name} User</td>
						<td class="w3-center">Total of all ${listCases.name} User</td>
  			<td class="w3-center">Total of all ${listCases.name} User</td>
    	...
    	<td class="w3-center">Total of all ${listCases.name} User</td>
  			...
  			<td class="w3-center" >Total of all ${listCases.name} User</td>
    			...
    			<td class="w3-center">Total of all period of ${listCases.name} User
    			
    			<span  onclick="getCaseStudiedTableByUserName(this,'getCasesProcessedExcelCOMM',1)" class="fa fa-download  w3-right w3-hover-blue"></span>
...
<td class="w3-center firstTd bus${listCases.name}" colorName="${listCases.color}">${listCases.location}</td>
											 ...
											 <c:choose>
        <c:when test="${fn:contains(FunctionAppBean.getDisplayBy(), '7')}">
		<td class="w3-center" style='cursor:pointer' onClick="getCaseStudiedTableByStageNameRow(this,'getCasesProcessedCOMM',0);">${listCases.day1}</td>
										<td style='cursor:pointer' class="w3-center" onClick="getCaseStudiedTableByStageNameRow(this,'getCasesProcessedCOMM',0);">${listCases.day2}</td>
											<td style='cursor:pointer' class="w3-center"  onClick="getCaseStudiedTableByStageNameRow(this,'getCasesProcessedCOMM',0);">${listCases.day3}</td>
													<td style='cursor:pointer' class="w3-center"  onClick="getCaseStudiedTableByStageNameRow(this,'getCasesProcessedCOMM',0);">${listCases.day4}</td>
										<td style='cursor:pointer' class="w3-center"  onClick="getCaseStudiedTableByStageNameRow(this,'getCasesProcessedCOMM',0);">${listCases.day5}</td>
											<td class="w3-center" style='cursor:pointer' onClick="getCaseStudiedTableByStageNameRow(this,'getCasesProcessedCOMM',0);">${listCases.day6}</td>
						
							<td style='cursor:pointer' class="w3-center"  onClick="getCaseStudiedTableByStageNameRow(this,'getCasesProcessedCOMM',0);">${listCases.day7}</td>
						
    </c:when>  
        <c:when test="${fn:contains(FunctionAppBean.getDisplayBy(), '4')}">
   		<td style='cursor:pointer' class="w3-center"  onClick="getCaseStudiedTableByStageNameRow(this,'getCasesProcessedCOMM',0);">${listCases.week1}</td>
										<td style='cursor:pointer' class="w3-center"  onClick="getCaseStudiedTableByStageNameRow(this,'getCasesProcessedCOMM',0);">${listCases.week2}</td>
											<td style='cursor:pointer' class="w3-center"  onClick="getCaseStudiedTableByStageNameRow(this,'getCasesProcessedCOMM',0);">${listCases.week3}</td>
													<td style='cursor:pointer' class="w3-center"  onClick="getCaseStudiedTableByStageNameRow(this,'getCasesProcessedCOMM',0);">${listCases.week4}</td>
											
											
    </c:when>  
        <c:when test="${fn:contains(FunctionAppBean.getDisplayBy(), '3')}">
   		<td style='cursor:pointer' class="w3-center"  onClick="getCaseStudiedTableByStageNameRow(this,'getCasesProcessedCOMM',0);">${listCases.period1}</td>
										<td style='cursor:pointer' class="w3-center"  onClick="getCaseStudiedTableByStageNameRow(this,'getCasesProcessedCOMM',0);">${listCases.period2}</td>
											<td style='cursor:pointer' class="w3-center"  onClick="getCaseStudiedTableByStageNameRow(this,'getCasesProcessedCOMM',0);">${listCases.period3}</td>
    </c:when>  
								...
								<u class="w3-hover-blue w3-hover-text-black"  onClick="getCaseStudiedTableByStageName(this,'getCasesProcessedCOMM',0);"
								...
								<span onClick="getCaseStudiedTableByStageName(this,'getCasesProcessedExcelCOMM',1)"  class="fa fa-download generateXLS w3-right"></span>
  			 ...
  			 <c:choose>
        <c:when test="${fn:contains(FunctionAppBean.getDisplayBy(), '7')}">
          <td class="w3-center" onClick="getCaseStudiedTableByTotalRow(this,'getCasesProcessedCOMM',0);" >Total of all Users</td>
  			<td class="w3-center" onClick="getCaseStudiedTableByTotalRow(this,'getCasesProcessedCOMM',0);" >Total of all Users</td>
  			<td class="w3-center" onClick="getCaseStudiedTableByTotalRow(this,'getCasesProcessedCOMM',0);" >Total of all Users</td>
  				<td class="w3-center" onClick="getCaseStudiedTableByTotalRow(this,'getCasesProcessedCOMM',0);" >Total of all Users</td>   
  			<td class="w3-center" onClick="getCaseStudiedTableByTotalRow(this,'getCasesProcessedCOMM',0);" >Total of all Users</td>
      ...
      <td class="w3-center"  onClick="getCaseStudiedTableByTotalRow(this,'getCasesProcessedCOMM',0);" >Total of all Users</td>
  			...
  			<td class="w3-center" onClick="getCaseStudiedTableByTotalRow(this,'getCasesProcessedCOMM',0);" >Total of all Users</td>
    			...
    			<td class="w3-center" onClick="getCaseStudiedTableByTotal(this,'getCasesProcessedCOMM',0);" >Total of all column on totals  
 ...
 $(".busi").each(function(){
	...
	for(var count=1;count<totalTd;count++){
		...
		var contEachTd=0;
		 ...
		 if(count==totalTd-1)
		 $(this).parent().find('td:eq('+count+')').append("<br><a style='cursor:pointer'  onclick=\"getCaseStudiedTableByUserName(this,'getCasesProcessedCOMM',0);\" >"+contEachTd+"</a>");
		 else
			 $(this).parent().find('td:eq('+count+')').append("<br><a style='cursor:pointer' onclick=\"getCaseStudiedTableByUserNameRow(this,'getCasesProcessedCOMM',0);\" >"+contEachTd+"</a>");
	...
	for(var count=1;count<totalTd;count++){
		...
		var contEachTd=0;
		 $(".busi").each(function(){
			...
			var textOfCell=$(this).parent().find('td:eq('+count+')').text();
			
		contEachTd+= parseInt(textOfCell.split("User")[1]);
...
function getCaseStudiedTableByStageName(thisVariable,ajaxUrl,download){
	$("#caseTable").html("");
...
//	 var  secondColumnValue = $(thisVariable).closest("tr").find("td:eq(1)").text();
  ...
  var process=$(thisVariable).closest('table').attr("process").trim();
...
function getCaseStudiedTableByStageNameRow(thisVariable,ajaxUrl,download){
	$("#caseTable").html("");
	 // alert("table");
...
/* function UserCount(){
	...
	var countData=[];
	 ...
	 $(".busi").each(function(){
			  ...
			  countData.push(parseInt($(this).last().text()));
